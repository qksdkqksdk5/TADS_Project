/* eslint-disable */
// ✅ 파일 위치: src/modules/stats/components/StatsPanel.jsx
// 변경사항: axios 직접 호출 → ../api.js 로 위임
...
function StatsPanel({ isMobile, host }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const response = await fetchStatsSummary(host, dataMode); // ✅ api.js
        setStats({
  ...
  useEffect(() => {
    if (viewMode === "history") {
      const load = async () => {
        try {
          const response = await fetchStatsHistory(host, selectedDate, dataMode); // ✅ api.js
    ...
    <div style={mainContainerStyle}>
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
              {["real", "sim", "all"].map((mode) => (
                <button key={mode} onClick={() => setDataMode(mode)}
                  ...
                  {mode === "real" ? "실제 데이터" : mode === "sim" ? "시뮬레이션" : "전체 보기"}
                </button>
              ))}
            </div>
          </div>
      ...
      {viewMode === "summary" ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ ...panelWrapper, padding: '25px', border: '1px solid #1e293b', background: 'linear-gradient(135deg, #0f172a 0%, #1e1b4b 100%)', marginTop: '10px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          <div style={filterBar}>
            ...
            <span style={{ color: '#475569', fontSize: '12px', marginLeft: '10px' }}>
              필터: {dataMode === 'real' ? '실제상황만' : dataMode === 'sim' ? '시뮬레이션만' : '전체 기록'}
            </span>
          </div>
                ...
                <div key={log.id} style={historyCardStyle}>
                  ...
                  <div style={historyInfo}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <span style={resolvedByStyle}>👤 {log.resolved_by ? log.resolved_by + ' 관리자' : '시스템'}</span>
                    </div>
