...
export default function RaspiModule() {
  ...
  const SERVER_IP = "localhost";        // 윈도우 분석 서버 IP
  const RASPI_IP = "192.168.219.155";   // 라즈베리파이 IP
  const SERVER_PORT = "5000";
  
  const BASE_URL = `http://${SERVER_IP}:${SERVER_PORT}/api/raspi`;
  
  // 1. 라즈베리파이에서 직접 오는 순수 일반 영상 (RGB)
  const NORMAL_VIDEO_URL = `http://${RASPI_IP}:5000/video_feed?t=${streamKey}`;
  // 2. 윈도우 분석 서버에서 오는 열화상 전용 영상 (순수 Thermal)
  ...
  const move = (x = 0, y = 0) => {
    ...
    fetchStatus(gcode);
    setPanPos(p => p + x); setTiltPos(p => p + y);
  };
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div style={{ display: 'flex', gap: '20px' }}>
            ...
            <div style={{...videoWrapperStyle, border: fireAlert ? '3px solid #ff4444' : '1px solid #2a2a3a'}}>
              ...
              />
              {fireAlert && (
                <div style={heatOverlayStyle}>⚠️ HEAT</div>
              )}
            </div>
          </div>
...
const videoWrapperStyle = {
  position: 'relative', width: '480px', height: '360px', background: '#000', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 10px 30px rgba(0,0,0,0.5)'
};
...
const Btn = ({ text, onClick, bg = '#2a2a4a', active = false }) => (
  <button onClick={onClick} style={{ width: '85px', height: '85px', background: active ? '#312e81' : bg, color: '#fff', border: 'none', borderRadius: '22px', cursor: 'pointer', fontSize: '28px', boxShadow: '0 6px 0 #000', transition: '0.1s' }}
