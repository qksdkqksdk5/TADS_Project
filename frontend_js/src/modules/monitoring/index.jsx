/* eslint-disable */
// src/modules/carbon/index.jsx
import { useState } from 'react';
import axios from 'axios';

export default function CarbonModule({ host }) {
  ...
  const checkHealth = async () => {
    ...
    setStatus(null);
    try {
      const res = await axios.get(`http://${host || window.location.hostname}:5000/api/carbon/health`);
  ...
  return (
    <div style={{ flex: 1, padding: '32px', background: '#0f0f1a', minHeight: '100vh', color: '#e0e0ff' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto', border: '1px dashed #2d4d3d', borderRadius: '12px', padding: '48px', textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: '24px', fontWeight: 700, marginBottom: '8px', color: '#fff' }}>탄소배출 / 정체 현황</h1>
        ...
        <div style={{ marginBottom: '32px' }}>
          ...
          {status === 'ok'    && <div style={{ marginTop: '12px', color: '#22c55e', fontSize: '13px', fontWeight: 600 }}>✅ 연결 성공! <code style={{ color: '#86efac' }}>/api/carbon/health</code> 응답 OK</div>}
        ...
        <div style={{ background: '#1a1a2e', borderRadius: '8px', padding: '24px', textAlign: 'left', fontSize: '13px', color: '#8080b0' }}>
          ...
          <ul style={{ paddingLeft: '20px', lineHeight: '2' }}>
            <li>백엔드: <code>backend_flask/modules/carbon/</code> 폴더에 라우트 추가</li>
            <li>프론트: 이 파일(<code>src/modules/carbon/index.jsx</code>)에 UI 개발</li>
            <li>API 연동: <code>src/modules/carbon/api.js</code> 파일 생성 후 axios 호출</li>
