/* eslint-disable */
// ✅ 파일 위치: src/modules/traffic/components/ControlPanel.jsx
// 변경사항: 없음 (경로 이동만)
...
const ControlPanel = ({ activeTab, startSim, logs }) => {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', height: '100%', minHeight: 0 }}>
      ...
      <div style={{ flex: 1, background: '#111827', padding: '10px', borderRadius: '12px', border: '1px solid #1e293b', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        <div style={{ fontSize: '13px', fontWeight: 'bold', color: '#cbd5e1', marginBottom: '10px', flexShrink: 0 }}>📡 실시간 시스템 로그</div>
        <div style={{ flex: 1, background: '#020617', padding: '12px', borderRadius: '8px', overflowY: 'auto', minHeight: 0 }}>
          {logs.length === 0 ? (
            <div style={{ color: '#475569', fontSize: '11px', fontFamily: 'monospace' }}>[시스템] 모니터링 중...</div>
          ) : (
            logs.map((log, i) => (
              <div key={i} style={{ color: log.includes('🚨') ? '#f87171' : '#4ade80', fontSize: '11px', marginBottom: '4px', fontFamily: 'monospace', lineHeight: '1.4' }}>
                {log}
              </div>
            ))
          )}
