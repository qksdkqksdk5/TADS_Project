...
const SingleMedia = ({ url, isHls, name, isFlashing, onToggle, isOn, showToggle }) => {
  ...
  return (
    <div style={gridItemStyle}>
      {isHls ? (
        ...
        <img
          src={url}
          style={{ ...mediaStyle, filter: isFlashing ? 'brightness(2.5)' : 'none', transition: 'filter 0.15s ease' }}
          alt="streaming"
        />
      )}
          ...
          <div style={modalContentStyle} onClick={(e) => e.stopPropagation()}>
            <div style={{ position: 'relative', width: '95vw', height: '90vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#000', borderRadius: '8px', overflow: 'hidden' }}>
              <div style={{ width: '100%', height: '100%' }}>
                <SingleMedia url={expandedMedia.url} isHls={expandedMedia.isHls} name={expandedMedia.name} />
              </div>
...
const mediaStyle         = { maxWidth: '95%', maxHeight: '95%', width: 'auto', height: 'auto', objectFit: 'contain', display: 'block', boxShadow: '0 0 15px rgba(0,0,0,0.5)', borderRadius: '2px' };
