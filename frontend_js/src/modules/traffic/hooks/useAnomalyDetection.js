...
export function useAnomalyDetection(socket, activeTab, setActiveTab, setVideoUrl, host, adminName) {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const response = await axios.get(`${getOrigin(host)}/api/pending_alerts`);
  ...
  const resolveEmergency = async (alertId, type, address, originType, isSimulation = false) => {
    try {
      ...
      await axios.post(`${getOrigin(host)}/api/resolve_alert_db`, {
  ...
  const resolveAllAlertsAction = async (alerts, isSimulation = false) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        await axios.post(`${getOrigin(host)}/api/resolve_alerts_bulk`, { 
          alertIds: alertIds, 
          isCorrect: 1, 
          adminName: adminName
        });
  ...
  useEffect(() => {
    ...
    const handleForceStart = (data) => {
      ...
      // setActiveTab("sim");
      setTimeout(() => {
        const syncUrl = `${getOrigin(host)}/api/video_feed?type=${data.type}&v=${Date.now()}`;
        setVideoUrl(syncUrl);
      }, 1500);
    };
    ...
    const handleAnomaly = async (data) => {
      ...
      const processAlert = async (finalAddress) => {
        ...
        if (!data.video_origin.includes('realtime_its')) {
          axios.post(`${getOrigin(host)}/api/update_address`, { 
        ...
        if (result.isConfirmed && !isCurrTabSim) {
          ...
          if (isSimulationValue && typeof setVideoUrl === 'function') {
            ...
            setTimeout(() => {
              const syncUrl = `${getOrigin(host)}/api/video_feed?type=${select_type}&v=${Date.now()}`;
              setVideoUrl(syncUrl);
            }, 1000);
          }
        }
