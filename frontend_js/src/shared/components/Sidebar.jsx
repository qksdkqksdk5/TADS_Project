...
function Sidebar({ activeTab, onTabChange, user, onLogout, isMobile }) {
  ...
  const menuItems = [
    { id: "cctv",       label: "CCTV Anomaly Detection", icon: "📡" },
    { id: "monitoring", label: "Traffic Flow Monitoring",   icon: "🚦" },
    { id: "tunnel", label: "Smart Tunnel System",
      icon: ( <img src="/tunnel.jpg" alt="Tunnel Icon" style={{ width: '20px', height: '20px', verticalAlign: 'middle', marginLeft: '1px', marginRight: '3px'}} />)
    },
    { id: "raspi",      label: "Raspberry Pi CCTV", icon: "🖥️" },
    { id: "plate",      label: "Auto License Plate Recognition",      icon: "🔍" },
    { id: "stats",      label: "Statistics & Reports",    icon: "📊" },
  ];

  const sidebarWidth = isMobile ? '100%' : (isCollapsed ? '80px' : '240px');

  return (
    <aside style={{ ...sidebarStyle, width: sidebarWidth, minWidth: isMobile ? 'auto' : sidebarWidth, height: isMobile ? 'auto' : '100vh', transition: 'width 0.35s cubic-bezier(0.4, 0, 0.2, 1)', position: 'relative' }}>
      {!isMobile && (
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden' }}>
        <div style={{ ...logoArea, padding: isMobile ? '12px 15px' : '30px 20px', justifyContent: isCollapsed ? 'center' : 'flex-start' }}>
          <div style={{ ...logoIcon, width: isMobile ? '32px' : '40px', height: isMobile ? '32px' : '40px', minWidth: isMobile ? '32px' : '40px' }}>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" style={{ width: isMobile ? '18px' : '24px', color: '#6366f1' }}>
        ...
        <nav style={{ ...sideNavStyle, flexDirection: isMobile ? 'row' : 'column', padding: isMobile ? '0 10px 10px 10px' : '10px 0' }}>
          ...
          {menuItems.map((menu) => (
            <div key={menu.id} onClick={() => onTabChange(menu.id)} style={{
              ...
              <span style={{ marginRight: (isCollapsed || isMobile) ? '0px' : '12px', fontSize: isMobile ? '18px' : '20px', width: isCollapsed ? '100%' : 'auto', textAlign: 'center' }}>
                {menu.icon}
              </span>
              {(!isCollapsed || isMobile) && (
      ...
      {!isMobile && (
        <div style={{ ...sidebarFooter, textAlign: isCollapsed ? 'center' : 'left', padding: isCollapsed ? '20px 10px' : '20px 25px' }}>
          {isCollapsed ? (
          ...
          <button onClick={onLogout} style={{ ...logoutBtn, fontSize: isCollapsed ? '10px' : '13px', padding: isCollapsed ? '8px 0' : '10px', marginTop: isCollapsed ? '0' : '5px' }}>
