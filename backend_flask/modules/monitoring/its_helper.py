...
ROAD_CONFIG = {
    ...
    'gyeongin': {
        ...
        # Overpass 요청 bbox — 두 노선을 모두 포함하는 넓은 범위로 가져온 뒤 아래 클리핑으로 정제
        'bounds': {'minX': 126.6, 'maxX': 127.1, 'minY': 37.3, 'maxY': 37.6},
        'type': 'ex',
        # 이름별 좌표 클리핑 — 서울외곽순환고속도로로 이어지는 동쪽 연결로(motorway_link)를 제거
        # 캐시 분석 결과:
        #   경인고속도로   본선 최대 경도 126.8349 → maxX 126.88 로 연결로 차단
        #   제2경인고속도로 본선 최대 경도 126.92, 이후 126.97~127.1 구간 40개 way 가 연결로
        #                  → maxX 126.94 로 갭(126.92↔126.97) 사이에서 깔끔하게 차단
        'osm_name_bounds': {
